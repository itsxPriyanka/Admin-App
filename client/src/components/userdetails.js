...
//       try {
//         const response = await fetch("https://jck6sradac.execute-api.ap-south-1.amazonaws.com/dev/users");
//         if (!response.ok) {
...
//             email: attributes.find(attr => attr.Name === 'email')?.Value || 'N/A',
//             createdDate: new Date(user.UserCreateDate).toLocaleDateString(),
//             status: user.Enabled ? 'Active' : 'Inactive',
//             emailVerified: attributes.find(attr => attr.Name === 'email_verified')?.Value === 'true' ? 'Yes' : 'No',
//             confirmationStatus: user.ConfirmationStatus === 'CONFIRMED' ? 'Confirmed' : 'Unconfirmed',
//           };
//         });
